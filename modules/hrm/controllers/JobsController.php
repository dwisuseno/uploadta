...
use Yii;
use app\modules\hrm\models\Jobs;
use app\modules\hrm\models\JobsSearch;
...
class JobsController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $providerPositions = new \yii\data\ArrayDataProvider([
    ...
    * @return mixed
    */
    public function actionAddPositions()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('Positions');
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('action') == 'load' && empty($row)) || Yii::$app->request->post('action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formPositions', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
